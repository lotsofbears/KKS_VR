    ...
    internal static class TalkSceneExtras
    {
        ...
        internal static void AddTalkColliders(IEnumerable<ChaControl> charas)
        {
            ...
            foreach (var chara in charas)
            {
                ...
                for (var i = 0; i < _talkColliders.GetLength(0); i++)
                {
                    var target = chara.objBodyBone.transform.Find(_talkColliders[i, 0]);
                    if (target.Find(_talkColliders[i, 2]) == null)
                    {
                        var collider = CommonLib.LoadAsset<GameObject>(_talkColliders[i, 1], _talkColliders[i, 2], true, string.Empty);
        ...
        internal static void AddHColliders(IEnumerable<ChaControl> charas)
        {
            ...
            {
                if (chara == null) continue;
                for (var i = 0; i < _hColliders.GetLength(0); i++)
                {
                    var target = chara.objBodyBone.transform.Find(_hColliders[i, 0]);
                    if (target != null && target.Find(_hColliders[i, 2]) == null)
                    {
                        var collider = CommonLib.LoadAsset<GameObject>(_hColliders[i, 1], _hColliders[i, 2], true);
                        collider.transform.SetParent(target, false);
                    }
                }
            }
