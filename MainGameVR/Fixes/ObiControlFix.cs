...
namespace KKS_VR.Fixes
{
    ...
    public class ObiCtrlFix
    {
        ...
        public static ObiFluidRenderer _renderer;
        public static void OnHSceneEnd()
        {
            ...
            //VRPlugin.Logger.LogDebug($"ObiCtrlFix[OnHSceneEnd]");
        }
        public static void SetFluidsState(bool state)
        {
            _activeState = state;
            //VRPlugin.Logger.LogDebug($"ObiCtrlFix[SetFluidsState = {state}] \n{new StackTrace()}");
        }
        ...
        public static void SetRenderer(bool state)
        {
            if (_renderer == null)
            {
                _renderer = VR.Camera.GetComponent<ObiFluidRenderer>();
            }
        ...
        public static IEnumerable<CodeInstruction> AwakeCameraSubstitute(IEnumerable<CodeInstruction> instructions)
        {
            ...
            foreach (var code in instructions)
            {
                if (found && !done)
                {
                    ...
                    {
                        //VRPlugin.Logger.LogDebug($"AwakeCameraSubstitute[{code.opcode}][{code.operand}]");
        ...
        public static IEnumerable<CodeInstruction> DestroyCommandBufferCameraSubstitute(IEnumerable<CodeInstruction> instructions)
        {
            ...
            foreach (var code in instructions)
            {
                ...
                if (found && !done)
                {
                    ...
                    {
                        //VRPlugin.Logger.LogDebug($"OnEnableCameraSubstitute[{code.opcode}][{code.operand}]");
                        yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ObiCtrlFix), nameof(ObiCtrlFix.GetVRCamera)));
                        done = true;
                        continue;
                    }
        ...
        {
            //VRPlugin.Logger.LogDebug($"ObiFluidRendererOnEnable[SetForceRenderIntoTexture]");
            VR.Camera.GetComponent<UnityEngine.Camera>().forceIntoRenderTexture = true;
        }
        public static UnityEngine.Camera GetVRCamera()
        {
            //VRPlugin.Logger.LogDebug($"GetVRCameraCall[ObiBaseFluidRenderer]");
